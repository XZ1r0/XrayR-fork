...
install() {
    curl -sL "${FORK_RAW}/install.sh" -o /tmp/install-xrayr.sh && bash /tmp/install-xrayr.sh install
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    curl -sL "${FORK_RAW}/install.sh" -o /tmp/install-xrayr.sh && bash /tmp/install-xrayr.sh install $version
    if [[ $? == 0 ]]; then
...
--- https://github.com/XZ1r0/XrayR-fork ---
  ${green}0.${plain} 修改配置
————————————————
